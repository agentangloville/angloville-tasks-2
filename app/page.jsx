...
function EmailHistorySection({ task, currentUser, onResendEmail }) {
  ...
  return (
    <div className="border-t pt-4 mt-4" style={{ borderColor: '#e8eaed' }}>
      <div className="flex items-center gap-2 mb-3"><Mail size={16} style={{ color: '#5f6368' }} /><label className="text-sm font-medium" style={{ color: '#202124' }}>Powiadomienia email</label></div>
      <div className="p-3 rounded-lg mb-3" style={{ background: '#f8f9fa' }}><p className="text-xs font-medium" style={{ color: '#5f6368' }}>Zgłaszający: {task.submittedBy || 'Nieznany'}</p>{task.submitterEmail ? <p className="text-xs" style={{ color: '#1a73e8' }}>{task.submitterEmail}</p> : <p className="text-xs" style={{ color: '#ea4335' }}>Brak adresu email</p>}</div>
      {emailHistory.length > 0 && <div className="space-y-2 mb-3"><p className="text-xs font-medium" style={{ color: '#5f6368' }}>Historia:</p>{emailHistory.map((email, idx) => { const sender = TEAM_MEMBERS.find(m => m.id === email.sentBy); return <div key={email.id || idx} className="flex items-center gap-2 p-2 rounded-lg" style={{ background: email.success ? '#e6f4ea' : '#fce8e6' }}>{email.success ? <MailCheck size={16} style={{ color: '#34a853' }} /> : <MailX size={16} style={{ color: '#ea4335' }} />}<div className="flex-1 min-w-0"><p className="text-xs" style={{ color: '#5f6368' }}>{formatDateTime(email.sentAt)} przez {sender?.name?.split(' ')[0] || 'System'}</p></div></div>; })}</div>}
      {hasEmail && task.status === 'closed' && <div className="flex items-center gap-2"><button onClick={handleResend} disabled={resending} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-50" style={{ background: lastCompletedEmail?.success ? '#f1f3f4' : '#34a853', color: lastCompletedEmail?.success ? '#202124' : 'white' }}>{resending ? <Loader2 size={16} className="animate-spin" /> : <RefreshCw size={16} />}{lastCompletedEmail?.success ? 'Wyślij ponownie' : 'Wyślij email'}</button></div>}
    </div>
  );
}
...
export default function TaskApp() {
  ...
  const visibleTasks = tasks.filter(t => { if (t.status === 'pending') return false; if (filterMarket !== 'all' && t.market !== filterMarket) return false; if (filterPerson !== 'all' && !t.assignees?.includes(filterPerson)) return false; return true; });
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#f8f9fa' }}>
      <aside className="w-56 flex flex-col min-h-screen flex-shrink-0 border-r bg-white" style={{ borderColor: '#e8eaed' }}>
        <div className="p-4 border-b" style={{ borderColor: '#e8eaed' }}><img src="https://angloville.com/wp-content/themes/angloville/assets/images/logo.svg" alt="Angloville" className="h-7" /><p className="mt-1 text-xs" style={{ color: '#5f6368' }}>Marketing Tasks</p></div>
        <div className="p-3 border-b space-y-2" style={{ borderColor: '#e8eaed' }}><select value={filterMarket} onChange={(e) => setFilterMarket(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm border" style={{ borderColor: '#dadce0', color: '#202124' }}><option value="all">Wszystkie rynki</option>{MARKETS.map(m => <option key={m.id} value={m.id}>{m.icon} {m.name}</option>)}</select><select value={filterPerson} onChange={(e) => setFilterPerson(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm border" style={{ borderColor: '#dadce0', color: '#202124' }}><option value="all">Wszyscy</option>{TEAM_MEMBERS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
        ...
        <div className="p-2 flex-1">
          <div className="space-y-0.5">
            {pendingTasks.length > 0 && <button onClick={() => setActiveTab('pending')} className="w-full flex items-center justify-between px-3 py-2 rounded-full text-sm" style={{ background: activeTab === 'pending' ? '#fef7e0' : 'transparent', color: activeTab === 'pending' ? '#b06000' : '#202124' }}><div className="flex items-center gap-3"><AlertCircle size={18} style={{ color: '#fbbc04' }} /><span>Oczekujące</span></div><span className="font-medium" style={{ color: '#fbbc04' }}>{pendingTasks.length}</span></button>}
            <button onClick={() => { setActiveTab('tasks'); setFilterStatus('active'); }} className="w-full flex items-center justify-between px-3 py-2 rounded-full text-sm" style={{ background: activeTab === 'tasks' && filterStatus === 'active' ? '#e8f0fe' : 'transparent', color: activeTab === 'tasks' && filterStatus === 'active' ? '#1a73e8' : '#202124' }}><div className="flex items-center gap-3"><Filter size={18} style={{ color: '#1a73e8' }} /><span>Aktywne</span></div><span className="font-medium">{openTasks.length + longtermTasks.length}</span></button>
            <button onClick={() => { setActiveTab('tasks'); setFilterStatus('open'); }} className="w-full flex items-center justify-between px-3 py-2 rounded-full text-sm" style={{ background: activeTab === 'tasks' && filterStatus === 'open' ? '#e8f0fe' : 'transparent', color: '#202124' }}><div className="flex items-center gap-3 pl-2"><Circle size={16} style={{ color: '#4285f4' }} /><span>Otwarte</span></div><span style={{ color: '#5f6368' }}>{openTasks.length}</span></button>
            <button onClick={() => { setActiveTab('tasks'); setFilterStatus('longterm'); }} className="w-full flex items-center justify-between px-3 py-2 rounded-full text-sm" style={{ background: activeTab === 'tasks' && filterStatus === 'longterm' ? '#f3e8fd' : 'transparent', color: '#202124' }}><div className="flex items-center gap-3 pl-2"><Clock size={16} style={{ color: '#a142f4' }} /><span>Long-term</span></div><span style={{ color: '#5f6368' }}>{longtermTasks.length}</span></button>
            <button onClick={() => { setActiveTab('tasks'); setFilterStatus('closed'); }} className="w-full flex items-center justify-between px-3 py-2 rounded-full text-sm" style={{ background: activeTab === 'tasks' && filterStatus === 'closed' ? '#e6f4ea' : 'transparent', color: '#202124' }}><div className="flex items-center gap-3 pl-2"><CheckCircle size={16} style={{ color: '#34a853' }} /><span>Zamknięte</span></div><span style={{ color: '#5f6368' }}>{closedTasks.length}</span></button>
          </div>
          <div className="mt-4 mx-2 p-3 rounded-lg text-xs" style={{ background: '#f1f3f4' }}><p className="mb-1.5" style={{ color: '#5f6368' }}>Formularz EN:</p><button onClick={copyLink} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-200"><code className="flex-1 text-xs truncate" style={{ color: '#1a73e8' }}>/request</code>{copied ? <Check size={14} style={{ color: '#34a853' }} /> : <Copy size={14} style={{ color: '#5f6368' }} />}</button></div>
          <QuickLinksSection currentUser={currentUser} />
        </div>
        <div className="p-3 border-t" style={{ borderColor: '#e8eaed' }}><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-medium" style={{ background: currentMember?.color }}>{getInitials(currentMember?.name || '')}</div><div className="flex-1 min-w-0"><div className="text-sm font-medium truncate" style={{ color: '#202124' }}>{currentMember?.name?.split(' ')[0]}</div>{isManager && <div className="text-xs" style={{ color: '#5f6368' }}>Manager</div>}</div><button onClick={handleLogout} className="p-1.5 rounded-full hover:bg-gray-100" style={{ color: '#5f6368' }}><LogOut size={18} /></button></div></div>
      </aside>

      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b px-6 py-3 flex items-center justify-between" style={{ borderColor: '#e8eaed' }}>
          <div><h2 className="text-lg font-medium" style={{ color: '#202124' }}>{activeTab === 'pending' ? 'Oczekujące na akceptację' : filterStatus === 'active' ? 'Aktywne zadania' : filterStatus === 'open' ? 'Otwarte zadania' : filterStatus === 'longterm' ? 'Zadania long-term' : filterStatus === 'closed' ? 'Zamknięte zadania' : 'Wszystkie zadania'}</h2>{filterPerson !== 'all' && <p className="text-xs" style={{ color: '#5f6368' }}>Filtr: {TEAM_MEMBERS.find(m => m.id === filterPerson)?.name}</p>}</div>
          <div className="flex items-center gap-2"><button onClick={loadTasks} className="p-2 rounded-full hover:bg-gray-100" style={{ color: '#5f6368' }}><Loader2 size={18} className={loading ? 'animate-spin' : ''} /></button>{activeTab === 'tasks' && <button onClick={() => setShowNewTask(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm hover:shadow-md" style={{ background: '#1a73e8', color: 'white' }}><Plus size={18} /> Nowe zadanie</button>}</div>
        </header>
        <div className="flex-1 overflow-y-auto p-4">
          {activeTab === 'pending' ? <PendingView tasks={pendingTasks} approveTask={approveTask} deleteTask={deleteTask} currentUser={currentUser} /> : (
            <div className="max-w-4xl mx-auto">{filteredTasks.length === 0 ? <div className="text-center py-16"><CheckCircle size={48} className="mx-auto mb-4" style={{ color: '#34a853', opacity: 0.4 }} /><p style={{ color: '#5f6368' }}>Brak zadań do wyświetlenia</p></div> : <div className="space-y-1">{filteredTasks.map(t => <TaskItem key={t.id} task={t} isSelected={selectedTask?.id === t.id} onClick={() => handleSelectTask(t)} onStatusChange={(s) => updateTask(t.id, { status: s })} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} onDragEnd={handleDragEnd} isDragging={draggedTask?.id === t.id} dragOverId={dragOverId} currentUser={currentUser} readTimestamps={readTimestamps} seenTaskIds={seenTaskIds} />)}</div>}</div>
          )}
        </div>
      </main>
...
// TASK ITEM
// =============================================
function TaskItem({ task, isSelected, onClick, onStatusChange, onDragStart, onDragOver, onDrop, onDragEnd, isDragging, dragOverId, currentUser, readTimestamps, seenTaskIds }) {
  ...
  return (
    <aside className="w-[640px] bg-white border-l flex flex-col overflow-hidden flex-shrink-0" style={{ borderColor: '#e8eaed' }}>
      <div className="p-4 border-b flex items-center justify-between" style={{ borderColor: '#e8eaed' }}>
        <div className="flex items-center gap-2"><span className="text-xl">{market?.icon}</span><span className="text-sm font-medium" style={{ color: '#202124' }}>{market?.name}</span>{task.isExternal && <ExternalLink size={14} style={{ color: '#fbbc04' }} />}{task.language === 'en' && <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: '#e8f0fe', color: '#1a73e8' }}>🇬🇧</span>}</div>
        <div className="flex items-center gap-1">
          {task.language === 'en' && <TranslateButton task={task} />}
          {publicLink && <button onClick={copyPublicLink} className="p-2 rounded-full hover:bg-blue-50" style={{ color: linkCopied ? '#34a853' : '#1a73e8' }} title="Kopiuj link">{linkCopied ? <Check size={18} /> : <Link2 size={18} />}</button>}
      ...
      {publicLink && <div className="px-4 py-2 border-b flex items-center gap-2" style={{ background: '#e8f0fe', borderColor: '#c6dafc' }}><Link2 size={14} style={{ color: '#1a73e8' }} /><code className="flex-1 text-xs truncate" style={{ color: '#1a73e8' }}>{publicLink}</code><button onClick={copyPublicLink} className="text-xs px-2 py-1 rounded hover:bg-blue-100" style={{ color: '#1a73e8' }}>{linkCopied ? '✓ Skopiowano' : 'Kopiuj'}</button></div>}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {editing ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2"><button onClick={save} className="flex-1 py-2 rounded-lg font-medium text-sm" style={{ background: '#1a73e8', color: 'white' }}>Zapisz</button><button onClick={() => setEditing(false)} className="flex-1 py-2 rounded-lg text-sm" style={{ background: '#f1f3f4', color: '#5f6368' }}>Anuluj</button></div>
          </div>
        ) : (
          <div><h3 className="font-medium text-xl mb-3" style={{ color: '#202124' }}>{task.title}</h3><RichTextDisplay html={task.description} /></div>
        )}
        
        {task.links && <div><label className="block mb-2 text-xs font-medium" style={{ color: '#5f6368' }}>Linki</label><div className="rounded-lg border p-1" style={{ background: '#f8f9fa', borderColor: '#e8eaed' }}><ClickableLinks text={task.links} /></div></div>}
        
        {/* Załączniki zadania */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2"><Paperclip size={16} style={{ color: '#5f6368' }} /><label className="text-sm font-medium" style={{ color: '#202124' }}>Załączniki ({task.attachments?.length || 0})</label></div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {!hasUnread && task.comments?.length > 0 && <button onClick={() => onMarkUnread(task.id)} className="text-xs flex items-center gap-1 px-2 py-1 rounded hover:bg-yellow-50" style={{ color: '#b06000' }}><Inbox size={14} />Oznacz nieprzeczytane</button>}
          </div>
          ...
          {/* Załączniki do nowego komentarza */}
          ...
          <div className="flex gap-2 items-end">
            <div className="flex-1">
              <input type="text" value={comment} onChange={(e) => setComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addComment()} placeholder="Napisz komentarz..." className="w-full px-4 py-2.5 rounded-xl text-sm" style={{ background: '#f1f3f4', border: '1px solid #e8eaed' }} />
            </div>
        ...
        <div className="pt-4 border-t text-xs space-y-1" style={{ borderColor: '#e8eaed', color: '#9aa0a6' }}><p>Utworzono: {formatDateTime(task.createdAt)}</p>{task.createdBy && <p>Przez: {TEAM_MEMBERS.find(m => m.id === task.createdBy)?.name || task.createdBy}</p>}{task.publicToken && <p>ID: {task.publicToken}</p>}</div>
      </div>
    </aside>
  );
}
...
function NewTaskModal({ onClose, onSave, currentUser }) {
  const [form, setForm] = useState({ title: '', description: '', market: 'pl', status: 'open', assignees: [currentUser], comments: [], attachments: [] });
  const [uploading, setUploading] = useState(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" style={{ boxShadow: '0 24px 38px 3px rgba(0,0,0,.14)' }} onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b flex items-center justify-between" style={{ borderColor: '#e8eaed' }}><h3 className="text-lg font-medium" style={{ color: '#202124' }}>Nowe zadanie</h3><button onClick={onClose} style={{ color: '#5f6368' }}><X size={20} /></button></div>
        <div className="p-5 space-y-4">
          <div><label className="text-sm font-medium block mb-1.5" style={{ color: '#202124' }}>Tytuł *</label><input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg text-sm" style={{ borderColor: '#dadce0' }} placeholder="Co trzeba zrobić?" autoFocus /></div>
          <div><label className="text-sm font-medium block mb-1.5" style={{ color: '#202124' }}>Opis</label><RichTextEditor value={form.description} onChange={(v) => setForm({ ...form, description: v })} placeholder="Szczegóły zadania..." /></div>
          
          {/* Załączniki w nowym zadaniu */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium" style={{ color: '#202124' }}>Załączniki</label>
            ...
            {form.attachments.length === 0 && <p className="text-xs" style={{ color: '#9aa0a6' }}>Kliknij 📎 aby dodać załączniki</p>}
          </div>
          
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-sm font-medium block mb-1.5" style={{ color: '#202124' }}>Rynek</label><select value={form.market} onChange={(e) => setForm({ ...form, market: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg text-sm" style={{ borderColor: '#dadce0' }}>{MARKETS.map(m => <option key={m.id} value={m.id}>{m.icon} {m.name}</option>)}</select></div>
            <div><label className="text-sm font-medium block mb-1.5" style={{ color: '#202124' }}>Typ</label><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg text-sm" style={{ borderColor: '#dadce0' }}><option value="open">Otwarte</option><option value="longterm">Long-term</option></select></div>
          </div>
          <div><label className="text-sm font-medium block mb-2" style={{ color: '#202124' }}>Przypisz do</label><div className="flex flex-wrap gap-2">{TEAM_MEMBERS.map(m => <button key={m.id} onClick={() => toggle(m.id)} className="flex items-center gap-2 px-3 py-2 rounded-full border text-sm" style={{ borderColor: form.assignees.includes(m.id) ? '#1a73e8' : '#dadce0', background: form.assignees.includes(m.id) ? '#e8f0fe' : 'white', color: form.assignees.includes(m.id) ? '#1a73e8' : '#202124' }}><div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-medium" style={{ background: m.color }}>{getInitials(m.name)}</div><span>{m.name.split(' ')[0]}</span>{form.assignees.includes(m.id) && <Check size={14} />}</button>)}</div></div>
        </div>
        <div className="p-5 border-t flex justify-end gap-3" style={{ borderColor: '#e8eaed' }}><button onClick={onClose} className="px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-100" style={{ color: '#5f6368' }}>Anuluj</button><button onClick={save} className="px-5 py-2.5 rounded-lg text-sm font-medium hover:shadow-md" style={{ background: '#1a73e8', color: 'white' }}>Utwórz zadanie</button></div>
      </div>
    </div>
  );
}
