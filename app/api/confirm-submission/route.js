...
export async function POST(request) {
  try {
    const { to, name, taskTitle, publicToken } = await request.json();

    if (!to || !taskTitle || !publicToken) {
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [to],
      subject: `✅ Request received: ${taskTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; background-color: #f8f9fa;">
          <div style="max-width: 500px; margin: 40px auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            ...
            <div style="padding: 32px;">
              <p style="color: #666; margin: 0 0 8px 0; font-size: 14px;">Hi ${name || 'there'}!</p>
              
              <h2 style="color: #202124; margin: 0 0 24px 0; font-size: 20px; font-weight: 600;">
                ✅ Your request has been received
              </h2>
              
              <div style="background: #f8f9fa; border-radius: 12px; padding: 20px; border-left: 4px solid #34a853; margin-bottom: 24px;">
                <p style="color: #5f6368; margin: 0 0 4px 0; font-size: 12px; font-weight: 600; text-transform: uppercase;">
                  Request
                </p>
                <p style="color: #202124; margin: 0; font-size: 16px; font-weight: 500;">
                  ${taskTitle}
                </p>
              </div>
              
              <p style="color: #5f6368; margin: 0 0 24px 0; font-size: 14px; line-height: 1.6;">
                The marketing team will review your request. You can track the status and communicate with the team using the link below.
              </p>
              
              <div style="background: #e8f0fe; border-radius: 12px; padding: 20px; margin-bottom: 24px; border: 1px solid #c2d7f7;">
                <p style="color: #1a73e8; margin: 0 0 8px 0; font-size: 14px; font-weight: 600;">
                  📍 Track Your Request
                </p>
                <p style="color: #5f6368; margin: 0 0 16px 0; font-size: 13px;">
                  Bookmark this link to check status and reply to the team:
                </p>
                <p style="background: white; padding: 12px 16px; border-radius: 8px; margin: 0; font-family: monospace; font-size: 12px; word-break: break-all; color: #1a73e8; border: 1px solid #dadce0;">
                  ${trackingUrl}
                </p>
              </div>
              ...
              <p style="color: #9aa0a6; margin: 24px 0 0 0; font-size: 12px;">
                💡 Pro tip: Reply to any team comments directly from the tracking page.
              </p>
            </div>
