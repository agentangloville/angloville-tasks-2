...
// Update all sends in a series (parent + children) — content only, not individual dates
...
export async function deleteThisAndFuture(send) {
  ...
  // Delete children with send_date >= this send's date
  const { error } = await supabase.from('scheduled_sends').delete()
    .eq('parent_id', pid).gte('send_date', send.sendDate);
  // If this IS the parent, also delete the parent if its date matches
  if (send.id === pid) {
...
export async function generateRecurrences(parentSend) {
  ...
  while (cur <= end) {
    const occ = await createScheduledSend({
      ...
      notes: parentSend.notes, links: parentSend.links, taskLink: parentSend.taskLink,
...
function mapSend(row) {
  return {
    ...
    notes: row.notes, links: row.links || [], taskLink: row.task_link || '',
